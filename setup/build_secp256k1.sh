...
(cd src && ./configure --host=x86_64-apple-darwin CC=`xcrun -find clang` CFLAGS="-O3 -arch x86_64 -isysroot `xcrun -sdk macosx --show-sdk-path` -fembed-bitcode-marker -mmacosx-version-min=10.9" CXX=`xcrun -find clang++` CXXFLAGS="-O3 -arch x86_64 -isysroot `xcrun -sdk macosx --show-sdk-path` -fembed-bitcode-marker -mmacosx-version-min=10.9" --prefix="$TARGETDIR_MACOSX" && make install --disable-dependency-tracking)
...
xcrun lipo -create "$TARGETDIR_MACOSX/lib/libsecp256k1.a" \
                    -o "$SCRIPT_DIR/../Libraries/secp256k1/lib/libsecp256k1.a" 
